...
for ieye=1:2
    ...
    Veye = eyeIn.([eye 'GazePointValidity']);
    data = eyeIn.([eye 'GazePointInUserCoordinateSystem']);  % Your data cell array
    eyeZ = zeros(size(data));  % Initialize array for Z-coordinates

    for i = 1:length(data)
        coordStr = data{i};  % Get the string from cell
        coordStr = coordStr(2:end-1);  % Removing parentheses
        coords = strsplit(coordStr, ',');  % Splitting string into separate numbers
        eyeZ(i) = str2double(coords{3});  % Converting the third string to number and storing in array
    end

    deye = smoothDistance(eyeZ,fsample,1); 
    
    data = eyeIn.([eye 'GazePointOnDisplayArea']);  % Your data cell array
    xorig = zeros(1,length(data));
    yorig = zeros(1,length(data));

    for i = 1:length(data)
        coordStr = data{i};  % Get the string from cell
        coordStr = coordStr(2:end-1);  % Removing parentheses
        coords = strsplit(coordStr, ',');  % Splitting string into separate numbers
        xorig(i) = str2double(coords{1});  % Converting the first string to number and storing in array
        yorig(i) = str2double(coords{2});  % Converting the second string to number and storing in array
    end

    dilat = eyeIn.([eye 'PupilDiameter']);
    dilat (dilat <= 0) = NaN;
    if ieye==1
...
deviceTimeStamp = eyeIn.TobiiSystemTimeStamp;
systemTimeStamp = eyeIn.FrameStartUnity;
if doUpsample
